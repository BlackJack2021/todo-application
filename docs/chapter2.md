# Chapter2. FastAPI を用いた Rest API の構築

## 1. 本章のスコープ

本章では主に FastAPI を用いた API 構築に焦点を当てて、Chapter1 で設計した API を構築します。ただし本章ではまだデータベースについては実装せず、python のリストを使ってデータを保存します。
...
どちらが適切かはプロジェクトの条件に応じて判断いただければと思います。今回はアプリケーションの機能ごとに切り分けるアプローチ(**モジュラーモノリス**)で実装したいと思います。
